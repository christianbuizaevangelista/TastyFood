...
// =============================================================================
// Retail Distributors (separate market segment — tracked only in Financials)
// =============================================================================
...
// GET /accounting/retail — list distributors with their outstanding balance.
accountingRouter.get(
  '/retail',
  asyncHandler(async (_req, res) => {
    const distributors = await prisma.retailDistributor.findMany({ orderBy: { name: 'asc' } });
    const sums = await prisma.retailTransaction.groupBy({ by: ['distributorId'], _sum: { charge: true, payment: true } });
    const byId = new Map(sums.map((s) => [s.distributorId, round2((s._sum.charge ?? 0) - (s._sum.payment ?? 0))]));
    res.json({ distributors: distributors.map((d) => ({ ...d, balance: byId.get(d.id) ?? 0 })) });
  })
);
...
// POST /accounting/retail — add a retail distributor.
accountingRouter.post(
  '/retail',
  asyncHandler(async (req, res) => {
    const b = retailSchema.parse(req.body);
    const d = await prisma.retailDistributor.create({
      data: { name: b.name, contactPerson: b.contactPerson ?? null, phone: b.phone ?? null, address: b.address ?? null, notes: b.notes ?? null, createdById: req.auth!.sub },
    });
    res.status(201).json(d);
  })
);

// PATCH /accounting/retail/:id — edit a retail distributor.
accountingRouter.patch(
  '/retail/:id',
  asyncHandler(async (req, res) => {
    const b = retailSchema.partial().parse(req.body);
    const existing = await prisma.retailDistributor.findUnique({ where: { id: req.params.id } });
    if (!existing) throw notFound('Distributor not found');
    const d = await prisma.retailDistributor.update({ where: { id: existing.id }, data: b });
    res.json(d);
  })
);

// DELETE /accounting/retail/:id — remove a distributor and its transactions.
accountingRouter.delete(
  '/retail/:id',
  asyncHandler(async (req, res) => {
    const existing = await prisma.retailDistributor.findUnique({ where: { id: req.params.id } });
    if (!existing) throw notFound('Distributor not found');
    await prisma.retailDistributor.delete({ where: { id: existing.id } });
    res.json({ ok: true });
  })
);

// GET /accounting/retail/:id — distributor detail + transactions with running balance.
accountingRouter.get(
  '/retail/:id',
  asyncHandler(async (req, res) => {
    const d = await prisma.retailDistributor.findUnique({ where: { id: req.params.id } });
    if (!d) throw notFound('Distributor not found');
    const txns = await prisma.retailTransaction.findMany({
      where: { distributorId: d.id },
      orderBy: [{ date: 'asc' }, { createdAt: 'asc' }],
    });
    let bal = 0;
    const withBalance = txns.map((t) => {
      bal = round2(bal + t.charge - t.payment);
      return { ...t, balance: bal };
    });
    res.json({ distributor: d, transactions: withBalance.reverse(), balance: bal });
  })
);

const txSchema = z.object({
  date: z.coerce.date(),
  type: z.enum(['SALE', 'PAYMENT', 'ADJUSTMENT']).default('SALE'),
...
// POST /accounting/retail/:id/transactions — record a charge (sale) or payment.
accountingRouter.post(
  '/retail/:id/transactions',
  asyncHandler(async (req, res) => {
    const d = await prisma.retailDistributor.findUnique({ where: { id: req.params.id } });
    if (!d) throw notFound('Distributor not found');
    const b = txSchema.parse(req.body);
    const charge = round2(b.charge ?? 0);
    const payment = round2(b.payment ?? 0);
    if (charge === 0 && payment === 0) throw badRequest('Enter a charge or a payment amount');
    const t = await prisma.retailTransaction.create({
      data: { distributorId: d.id, date: b.date, type: b.type, description: b.description ?? null, reference: b.reference ?? null, charge, payment, createdById: req.auth!.sub },
    });
    res.status(201).json(t);
  })
);

// DELETE /accounting/retail/:id/transactions/:txId
accountingRouter.delete(
  '/retail/:id/transactions/:txId',
  asyncHandler(async (req, res) => {
    const t = await prisma.retailTransaction.findUnique({ where: { id: req.params.txId } });
    if (!t || t.distributorId !== req.params.id) throw notFound('Transaction not found');
    await prisma.retailTransaction.delete({ where: { id: t.id } });
    res.json({ ok: true });
  })
);

// GET /accounting/retail/:id/export — download this distributor's transactions as CSV.
accountingRouter.get(
  '/retail/:id/export',
  asyncHandler(async (req, res) => {
    const d = await prisma.retailDistributor.findUnique({ where: { id: req.params.id } });
    if (!d) throw notFound('Distributor not found');
    const txns = await prisma.retailTransaction.findMany({ where: { distributorId: d.id }, orderBy: [{ date: 'asc' }, { createdAt: 'asc' }] });
    let bal = 0;
    const rows = txns.map((t) => {
      bal = round2(bal + t.charge - t.payment);
      return [t.date.toISOString().slice(0, 10), t.type, t.description ?? '', t.reference ?? '', String(t.charge), String(t.payment), String(bal)];
    });
    const csv = ['Date,Type,Description,Reference,Charge,Payment,Balance', ...rows.map((r) => r.map(csvCell).join(','))].join('\n');
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename="${d.name.replace(/[^a-z0-9]+/gi, '_')}_transactions.csv"`);
    res.send(csv);
  })
);

// POST /accounting/retail/:id/import — bulk-add transactions from CSV text.
// Expected columns: Date,Type,Description,Reference,Charge,Payment (header optional).
accountingRouter.post(
  '/retail/:id/import',
  asyncHandler(async (req, res) => {
    const d = await prisma.retailDistributor.findUnique({ where: { id: req.params.id } });
    ...
    if (lines.length && /date/i.test(lines[0]) && /(charge|payment|type)/i.test(lines[0])) lines.shift();
    ...
    for (const line of lines) {
      ...
      const type = (c[1] || 'SALE').toUpperCase();
      const charge = Number(c[4]) || 0;
      const payment = Number(c[5]) || 0;
      if (charge === 0 && payment === 0) continue;
      data.push({
        distributorId: d.id,
        date,
        type: ['SALE', 'PAYMENT', 'ADJUSTMENT'].includes(type) ? type : 'SALE',
    ...
    if (!data.length) throw badRequest('No valid rows found in the file');
    await prisma.retailTransaction.createMany({ data });
