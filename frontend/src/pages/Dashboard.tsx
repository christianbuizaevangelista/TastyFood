  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${ROLE_LABEL[user!.role]} · ${user?.org.name} · your own sales, this month`}
      />
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4 xl:grid-cols-4">
        ...
        {/* Count of active accounts per tier, scoped to the role's downstream. */}
        {(user!.role === 'PRINCIPAL') && (
          <KpiCard label="Active Provincial" value={num(c.activePerformers.provincial)} hint="active · downstream" accent="text-brand-600" />
        )}
